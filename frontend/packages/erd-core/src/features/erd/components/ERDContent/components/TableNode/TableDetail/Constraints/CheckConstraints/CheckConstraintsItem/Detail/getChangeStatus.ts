...
export function getChangeStatus({
  ...
  // Check for constraint detail changes
  const constraintDetailDiffItem = filteredDiffItems.find((item) => {
    const parsed = safeParse(constraintDetailDiffItemSchema, item)
    return parsed.success && parsed.output.constraintId === constraintId
  })

  return constraintDetailDiffItem?.status ?? 'unchanged'
}
