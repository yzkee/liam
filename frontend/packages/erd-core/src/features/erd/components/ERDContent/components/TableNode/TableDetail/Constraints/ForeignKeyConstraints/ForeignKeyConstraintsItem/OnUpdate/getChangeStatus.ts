...
export function getChangeStatus({
  ...
  // Check for constraint onUpdate changes
  const constraintUpdateDiffItem = filteredDiffItems.find((item) => {
    const parsed = safeParse(constraintUpdateConstraintDiffItemSchema, item)
    return parsed.success && parsed.output.constraintId === constraintId
  })

  return constraintUpdateDiffItem?.status ?? 'unchanged'
}
